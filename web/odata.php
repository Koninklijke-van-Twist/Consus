 ...
 * (nightly/hourly/on-demand via odata_get_all / consus_each_url_live) naar Mímir i.p.v. BC.
 *
 * Met $mimirApi gezet zijn $auth_list / $environment / $baseUrl / $auth ongebruikt voor BC;
 * Mímir beheert environments — Consus heeft alleen de API-key (+ optioneel $mimirBase) nodig.
 *
...
function odata_mimir_request(string $method, string $path, ?array $jsonBody = null): array
{
    ...
    $opts = [
        ...
        CURLOPT_USERAGENT => 'Consus-MimirClient/1.0',
    ];
    if ($jsonBody !== null) {
...
function odata_get_all(string $url, array $auth, $ttlSeconds = 300): array
{
    ...
    if (odata_mimir_api_key() !== '') {
        // Mímir beheert de BC-cache (max_age); Consus-filecache / live-paginering worden overgeslagen.
