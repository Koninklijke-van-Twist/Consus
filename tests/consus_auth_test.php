...
$savedAuthFile = getenv('CONSUS_AUTH_FILE');
$savedGlobals = null;
$authFile = null;
...
PHP;
    ...
    if ($savedAuthFile === false) {
        putenv('CONSUS_AUTH_FILE');
    } else {
        putenv('CONSUS_AUTH_FILE=' . $savedAuthFile);
    }

    if (is_array($savedGlobals)) {
    ...
    if (is_string($authFile) && is_file($authFile)) {
        unlink($authFile);
    }
}
